...
// メンテナンスモードのON/OFF設定
define('MAINTENANCE_MODE', true); // true: メンテナンス中, false: 通常運用
...
/**
 * スマホでの再読み込み問題を解決する修正コード
 * functions.phpに追加するコード
 */

// スマホでの再読み込み防止用のJavaScriptを出力
function prevent_mobile_reload_script() {
    ?>
    <script>
    /**
     * スマホでの再読み込み防止修正
    ...
    console.log('🔄 再読み込み防止スクリプト開始');
    
    // ===== 1. リサイズによる自動リロードを完全に無効化 =====
    function disableAutoReload() {
        console.log('🚫 自動リロード無効化中...');
        
        // location.reloadを一時的に無効化
        var originalReload = window.location.reload;
        window.location.reload = function(force) {
            console.log('🛑 リロードがブロックされました:', new Error().stack);
            // リロードを実行しない
            return false;
        };
        ...
        allScripts.forEach(function(script) {
            if (script.textContent && script.textContent.indexOf('location.reload') !== -1) {
                console.log('🗑️ リロードを含むスクリプトを無効化');
                script.type = 'text/disabled';
            }
        });
        
        console.log('✅ 自動リロード無効化完了');
    }
    
    // ===== 2. window.addEventListener の resize イベントを制御 =====
    function controlResizeEvents() {
        console.log('📏 リサイズイベント制御中...');
        ...
        window.addEventListener = function(type, listener, options) {
            if (type === 'resize') {
                ...
                if (listenerString.indexOf('reload') !== -1 || 
                    listenerString.indexOf('location.reload') !== -1) {
                    console.log('🚫 リロードを含むリサイズイベントをブロック');
                    return; // 登録しない
                }
                
                // 安全なリサイズイベントのみ登録
                resizeListeners.push(listener);
                console.log('✅ 安全なリサイズイベントを登録');
            }
        ...
        console.log('✅ リサイズイベント制御完了');
    }
    
    // ===== 3. ページ遷移の防止 =====
    function preventUnwantedNavigation() {
        console.log('🚪 不要なページ遷移防止中...');
        ...
        window.addEventListener('popstate', function(e) {
            ...
            if (navigationCount > 3) {
                console.log('🚫 過度なpopstateをブロック');
                e.preventDefault();
                return false;
            }
        });
        
        console.log('✅ ページ遷移防止完了');
    }
    
    // ===== 4. フォーム送信による再読み込み防止 =====
    function preventFormReload() {
        console.log('📝 フォーム再読み込み防止中...');
        ...
        document.addEventListener('submit', function(e) {
            ...
            if (form.method === 'get' && !form.action) {
                console.log('🚫 意図しないフォーム送信をブロック');
                e.preventDefault();
                return false;
            }
        });
        
        console.log('✅ フォーム再読み込み防止完了');
    }
    
    // ===== 5. タッチイベントでのページ遷移防止 =====
    function preventTouchNavigation() {
        console.log('👆 タッチナビゲーション制御中...');
        ...
        document.addEventListener('touchmove', function(e) {
            ...
            // 上向きスワイプで再読み込みトリガーを防止
            if (deltaY > 50 && window.pageYOffset === 0) {
                console.log('🚫 プルリフレッシュをブロック');
                e.preventDefault();
            }
        ...
        console.log('✅ タッチナビゲーション制御完了');
    }
    
    // ===== 6. 既存のJavaScriptエラーハンドリング =====
    function handleJavaScriptErrors() {
        console.log('⚠️ JavaScriptエラーハンドリング設定中...');
        
        window.addEventListener('error', function(e) {
            console.log('JavaScript エラー:', e.error);
            
            // エラーによる再読み込みを防止
            if (e.error && e.error.message && 
                e.error.message.indexOf('reload') !== -1) {
                console.log('🚫 エラーによる再読み込みをブロック');
                e.preventDefault();
                return false;
            }
        });
        
        // unhandledrejection も監視
    ...
    // ===== 7. 特定のライブラリやスクリプトの制御 =====
    function controlProblematicScripts() {
        console.log('📜 問題のあるスクリプト制御中...');
        
        // 問題を起こす可能性のあるグローバル関数を制御
        if (window.jQuery) {
            var originalWindowResize = window.jQuery.fn.resize;
            if (originalWindowResize) {
                window.jQuery.fn.resize = function() {
                    console.log('🚫 jQuery resize イベントをブロック');
                    return this; // チェーンを維持
                };
            }
        }
        
        // setTimeout/setIntervalでのリロードを防止
        ...
        window.setTimeout = function(callback, delay) {
            if (typeof callback === 'function') {
                ...
                if (callbackString.indexOf('reload') !== -1) {
                    console.log('🚫 setTimeout内のリロードをブロック');
                    return;
                }
            }
        ...
        window.setInterval = function(callback, delay) {
            if (typeof callback === 'function') {
                var callbackString = callback.toString();
                if (callbackString.indexOf('reload') !== -1) {
                    console.log('🚫 setInterval内のリロードをブロック');
                    return;
                }
            }
        ...
        console.log('✅ 問題スクリプト制御完了');
    }
    
    // ===== メイン実行関数 =====
    function initReloadPrevention() {
        console.log('🚀 再読み込み防止初期化開始');
        
        function executeAllPrevention() {
            try {
                // 最重要：自動リロードを最初に無効化
                disableAutoReload();
                ...
                preventFormReload();
                preventTouchNavigation();
                handleJavaScriptErrors();
                controlProblematicScripts();
                
                console.log('🎉 全ての再読み込み防止が完了しました');
                
                // 5秒後に状況を確認
                setTimeout(function() {
                    console.log('📊 5秒後の状況確認 - URL:', window.location.href);
                }, 5000);
                
            } catch (error) {
                console.error('❌ 再読み込み防止中にエラー:', error);
            }
        }
        
        // 即座に実行（最重要）
        executeAllPrevention();
        
        // DOM読み込み後にも再実行
        if (document.readyState === 'loading') {
            document.addEventListener('DOMContentLoaded', executeAllPrevention);
        }
    }
...
// 最優先でスクリプトを出力（headの最初）
add_action('wp_head', 'prevent_mobile_reload_script', 1);

// 管理画面では実行しない
if (!is_admin()) {
    // 追加のmeta tagとCSS
    function add_reload_prevention_meta() {
        ?>
        <!-- 再読み込み防止用のmeta設定 -->
    ...
    add_action('wp_head', 'add_reload_prevention_meta', 1);
}